...
class Utils {
    ...
    static convertGRpcUtxosToRustUtxos(utxos) {
            ...
            return {
                ...
                utxoEntry: entry.utxoEntry && {
                    ...
                    scriptPublicKey: {
                        ...
                        script: this.hexStringToByteArray(entry.utxoEntry?.scriptPublicKey?.scriptPublicKey) || [],
                    },
                },
            };
        });
    }
    static convertRustTransactionToGRpcTransaction(transaction) {
        return {
            ...transaction,
            ...
            outputs: transaction.outputs.map((output) => {
                return {
                    scriptPublicKey: {
                        ...
                        scriptPublicKey: this.byteArrayToHexString(output.scriptPublicKey.script),
                    },
                    amount: output.value,
                    verboseData: undefined,
                };
    ...
    static async getXPublicKey(xprv, accountIndex) {
        return await kaspa_wasm_1.XPublicKey.fromXPrv(xprv, false, accountIndex);
    }
    static async getXPrivateKey(xprv, accountIndex) {
        return new kaspa_wasm_1.XPrivateKey(xprv, false, accountIndex);
    }
    /**
     ...
     */
    static async estimateFee({ utxos, outputs, }) {
        const rustUtxos = Utils.convertGRpcUtxosToRustUtxos(utxos);
        const utxoSet = kaspa_wasm_1.UtxoSet.from({ entries: rustUtxos });
        const amount = Utils.getOutputSum(outputs);
        const utxoSelection = await utxoSet.select(BigInt(amount), kaspa_wasm_1.UtxoOrdering.AscendingDaaScore);
        const inputs = utxoSelection.utxos.map((utxo, index) => {
            return new kaspa_wasm_1.TransactionInput({
        ...
        const payments = outputs.map((i) => new kaspa_wasm_1.PaymentOutput(new kaspa_wasm_1.Address(i.recipient.toString()), BigInt(i.amount)));
        const tx = new kaspa_wasm_1.Transaction({
            inputs,
            outputs: new kaspa_wasm_1.PaymentOutputs(payments),
            lockTime: 0,
            subnetworkId: [
        ...
        const estimatedFee = (0, kaspa_wasm_1.minimumTransactionFee)(tx, kaspa_wasm_1.NetworkType.Mainnet);
     ...
     */
    static async createTransaction({ utxoSet, outputs, changeAddress, fee = Wallet_1.Config.DEFAULT_FEE, priorityFee = 0, }) {
        const totalAmount = Utils.getOutputSum(outputs) + BigInt(fee) + BigInt(priorityFee);
        const utxoSelection = await utxoSet.select(totalAmount, kaspa_wasm_1.UtxoOrdering.AscendingDaaScore);
        ...
        // Build output
        const payments = outputs.map((i) => new kaspa_wasm_1.PaymentOutput(new kaspa_wasm_1.Address(i.recipient.toString()), BigInt(i.amount)));
        return (0, kaspa_wasm_1.createTransaction)(utxoSelection, new kaspa_wasm_1.PaymentOutputs(payments), new kaspa_wasm_1.Address(changeAddress.toString()), priorityFee);
    }
    /**
     * `signTransaction` calls `signTransaction` in WASM js to sign the `MutableTransaction`
     * object given an array of privateKeys
     */
    static signTransaction(tx, privateKeys) {
        return (0, kaspa_wasm_1.signTransaction)(tx, privateKeys, true);
    }
    static async sendTransaction({ clientProvider, utxos, privateKeys, outputs, changeAddress, fee = Wallet_1.Config.DEFAULT_FEE, priorityFee = 0, }) {
        if (utxos.length === 0) {
        ...
        const rpcTx = this.convertRustTransactionToGRpcTransaction(signedTx.toRpcTransaction());
    ...
    static getUtxosSum(utxos) {
        return utxos.reduce((acc, item) => {
            if (!item.utxoEntry?.amount)
                return acc;
            return BigInt(acc) + BigInt(item.utxoEntry.amount);
        }, BigInt(0));
    }
    static getOutputSum(outputs) {
        return outputs.reduce((acc, item) => BigInt(acc) + BigInt(item.amount || 0), BigInt(0));
    }
}
exports.Utils = Utils;
